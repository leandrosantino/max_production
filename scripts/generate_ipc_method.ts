            ...
            if (decorator && method.isAsync()) {
                ...
                return useQuery({
                    ...
                    queryFn: ({queryKey}):${returnType} => {
                        ...
                        return window.app.invoke('${methodName}', ${parameters})
                    }
                })
            }`
                    :
                ...
                return useMutation({
                    mutationFn: ( args:{${typedParameters} }):${returnType} => {
                        return window.app.invoke('${methodName}', ...Object.values(args))
                    }
                })
            }`
                ...
                content = '' +
                    `        ${methodName}: {
${query},
            invoke: async (${typedParameters}):${returnType} => await window.app.invoke('${methodName}', ${parameters})
        },
                `
...
function getRelativeType(typeText: string): string {
    const a = typeText.replace(/\("(.+?)"\)/g, (_, absolutePath) => {
        return `("${replacePathAlias(absolutePath, (aliasPath as any).replaceAll('*', ''), alias)}")`
    });
    return a
}
